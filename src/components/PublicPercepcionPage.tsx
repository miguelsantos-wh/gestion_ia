...
export default function PublicPercepcionPage({ routeHash }: PublicPercepcionPageProps) {
  ...
  const confirmPlacement = () => {
    ...
    savePerceptionPlacement(employee.id, evaluatorName, pending.performanceLevel, pending.potentialLevel);
    setDone(true);
  };
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 max-w-md text-center">
          <CheckCircle className="mx-auto text-teal-500 mb-3" size={40} />
          <h1 className="text-lg font-bold text-gray-900">Percepción registrada</h1>
          <p className="text-sm text-gray-600 mt-2">
            Has indicado dónde ubicarías a <strong>{employee.name}</strong> en la matriz 9-Box según tu percepción.
          </p>
          <button
            type="button"
            onClick={() => {
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-lg mx-auto">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900">Percepción 9-Box</h1>
            <p className="text-xs text-gray-500">Sin cuestionario: solo ubica en la cuadrícula</p>
          </div>
        </div>

        <p className="text-sm text-gray-700 mb-4 leading-relaxed">
          Evalúas a <strong>{employee.name}</strong>. Elige la casilla que mejor refleja <em>cómo te parece</em> que está
          en resultados (eje horizontal) y valores (vertical).
        </p>
        ...
        {pending && cfg && (
          <div className="bg-teal-50 border border-teal-100 rounded-xl p-4 mb-4">
            ...
            <p className="text-xs text-teal-800 mt-1">{cfg.description}</p>
            <button
              type="button"
              onClick={confirmPlacement}
              className="mt-3 w-full py-2.5 rounded-xl bg-teal-600 text-white text-sm font-semibold"
            >
              Confirmar y enviar
            </button>
          </div>
