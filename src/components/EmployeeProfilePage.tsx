...
function EvaluacionesTab({ emp }: { emp: EmployeeProfile }) {
  const lastEvent = emp.journey.filter(j => j.type === 'review').at(-1);
  return (
    <div className="space-y-5">
      <p className="text-base font-bold text-gray-900">Historial de evaluaciones</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Última evaluación', value: lastEvent?.date ?? '—' },
          { label: 'Resultado', value: lastEvent?.description?.split(':')[1]?.trim() ?? '—' },
          { label: 'Antigüedad', value: `${emp.tenure ?? '—'} años` },
        ].map(card => (
      ...
      <div className="space-y-3">
        {emp.journey.filter(j => j.type === 'review').map((ev, i) => (
